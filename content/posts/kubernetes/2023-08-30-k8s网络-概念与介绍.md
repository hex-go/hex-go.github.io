...
> 注：Cilium 是「超 CNI」——节点内数据路径用 eBPF（替换掉 iptables + bridge），跨节点传输仍选 VXLAN（Overlay）或 BGP（Underlay），故横跨两类；IPAM 也是自己的一套（cluster-pool / 云厂商集成），详见系列第⑤篇。

{{< expand "Cilium 与传统数据路径的对比" >}}
