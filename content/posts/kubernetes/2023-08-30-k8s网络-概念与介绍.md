...
- **直连路由**（`172.17.0.0/16`）：同网段流量走 eth0，目标MAC 从 ARP 缓存查
- **默认路由**（`default`）：其他流量走 eth0，下一跳是 docker0 的 IP（`172.17.0.1`），目标MAC = docker0 的 MAC

### 2.5 容器 A ping 容器 B（同主机）
