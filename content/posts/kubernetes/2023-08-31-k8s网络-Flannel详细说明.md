...
> 注意：Calico 的所有模式都不用网桥——包括 VXLAN 模式，它用 per-pod 路由 + VTEP 设备（`vxlan.calico`）做 Overlay，而不是把 veth 插进网桥。「用了 Overlay 就一定有网桥」是常见误区，详见 Calico 篇。
...
封装结构：<code>|HostMAC|<mark>HostIP|UDP</mark>|内层IP|Data|</code>，IP 头（20B）+ UDP 头（8B）= 28B 开销。
{{< /keypoint >}}
...
```text
S.容器 → S.cni0 → S.flanneld → D.flanneld → D.cni0 → D.容器
```

**1. S.容器 → S.cni0**

{{< keypoint >}}
默认路由 → veth pair → cni0 三层接口上交宿主机协议栈
{{< /keypoint >}}
...
{{< keypoint >}}
内核剥外层 L2/L3/L4 → flanneld 得原始 L3 报文 → 写回 flannel0（用户→内核）→ 路由 /24 dev cni0 → 出设备是网桥，内核 ARP 封 L2 头（src=cni0.MAC，dst=容器.MAC）→ cni0 CAM 转发
{{< /keypoint >}}
...
{{< keypoint >}}
cni0 CAM 查端口 → veth pair → 目标容器 eth0
{{< /keypoint >}}
...
|     [1] route: default->cni0.1     |                                |     [17] veth pair -> recv pkt      |
|         ARP:   D.MAC = cni0.MAC    |                                |                                     |
|         (user->kernel, 1st switch)    |                                |                                     |
|     [2] veth pair                     |            [2] [16]            |     [16] veth pair                  |
|     container.eth0 <-> cni0.port   |                                |     cni0 port <-> container eth0 |
|                                       |                                |                                     |
|   [S.cni0]  IP:10.244.1.1          |                                |   [D.cni0]  IP:10.244.2.1        |
...
|         -> flannel0 (flanneld preset) |                                |          in subnet -> cni0       |
|         kernel strips L2 hdr          |                                |       cni0 L2 bridge, add L2     |
...
| 编号 | 位置 | 规则 / 动作 |
|------|------|-----------|
| [1] | S.Container eth0 | **路由决策：** 查路由表，目标 `10.244.2.3` 不在本机子网 → 匹配默认路由，下一跳 = `cni0` IP(`10.244.1.1`)。发起 ARP 获取 `cni0` MAC，封装 L2 帧（目标 MAC = `cni0.MAC`，目标 IP = `10.244.2.3`）。**第一次状态切换：用户态→内核态。** |
| [2] | veth pair | 容器 eth0 → veth pair → cni0 网桥上的端口 vethxx。veth 插入网桥后降级为端口，只收不发，由 cni0 决定转发。 |
| [3] | S.cni0 | **目标 MAC 判断：** 收到帧 → 目标 MAC = 自身 MAC → **cni0 充当三层接口**，上交 S.Host 协议栈处理。 |
...
| [15] | D.Host net stack | **路由决策：** 目标 `10.244.2.3` ∈ 本机 Pod 子网 (`10.244.2.0/24`) → 出设备 = `cni0`。`cni0` 是二层网桥，**须先封装 L2 头** → 查 ARP 缓存（或发 ARP 请求）获取 D.Container MAC → 封装以太网帧头（源 MAC=`cni0.MAC`，目标 MAC=D.Container MAC）。 |
| [16] | D.cni0 | **CAM 转发：** 查 CAM 表，目标 MAC → 对应端口 veth → 从该端口发出。 |
...
封包格式：<code>|HostMAC|<mark>HostIP|UDP|VXLAN|内层MAC</mark>|内层IP|Data|</code>，总计 50B 开销（外层 IP 头 20 + UDP 头 8 + VXLAN 头 8 + 内层 MAC 14）。
{{< /keypoint >}}
...
```bash
# 示例：查看 FDB 表
bridge fdb show dev flannel.1 | grep <目标VTEP.MAC>
# <目标VTEP.MAC> dev flannel.1 dst 10.168.0.3 self permanent
# 含义：发往该 VTEP MAC 的二层帧，通过 flannel.1 设备发出，发往物理 IP=10.168.0.3 的节点
```

{{< keypoint >}}
**关键改进：** 封装/解封装全在内核态完成（通过 `flannel.1` 虚拟网卡），无需像 UDP 模式那样经过用户态 `flanneld` 进程，性能显著优于 UDP 方案。
{{< /keypoint >}}
...
{{< expand "知识科普：路由的 via（下一跳）不替换目的 IP" >}}

`via` 是路由表里「下一跳」的意思——**它不替换目的 IP，只决定本跳把包交给谁**。

以 `10.244.2.0/24 via 192.168.1.2 dev eth0` 处理目的 IP = 10.244.2.8 的包为例：

1. 最长前缀匹配 → 命中该路由 → 出接口 eth0，下一跳 = 192.168.1.2
2. 确定「发给谁」：有 via → 发给 via IP（192.168.1.2），**不是目的 IP**
3. ARP 查 192.168.1.2 的 MAC（不是 10.244.2.8 的 MAC）
4. 封 L2 帧：dst MAC = 192.168.1.2 的 MAC，src MAC = eth0 的 MAC
5. **L3 头原封不动**：dst IP 仍是 10.244.2.8

| 路由类型 | 下一跳 = 谁 | ARP 谁 | 例子 |
|---------|------------|--------|------|
| 直连（无 via） | 目的 IP 本身（on-link） | 目的 IP | `10.244.2.0/24 dev cni0` → ARP 10.244.2.8 |
| via 路由 | via 后面的 IP | via IP | `10.244.2.0/24 via 192.168.1.2` → ARP 192.168.1.2 |

**IP 路由 = 每跳重封 L2 帧，IP 头永不动。** 改 IP 只发生在 NAT（SNAT/DNAT），纯路由从不改 IP。「下一跳」是 IP 路由可扩展的根本：每台设备只需知道下一跳，不需要知道完整路径。

> VXLAN 里这条 `via 10.244.2.0 dev flannel.1 onlink` 是特例：via 用的是一个占位 IP，配合 `onlink` + proxy ARP，让内核在 flannel.1 上 ARP 得到的是**远端 VTEP 的 MAC**（见上面步骤3）。但「ARP 下一跳、不改 IP」的原则完全一致。
...
- `192.168.1.215.38381 > 192.168.1.216.8472`：外层 UDP 包，源端口 38381，目标端口 8472（VXLAN 协议端口）
- `f2:60:ca:96:7f:1f > 4a:02:36:26:f9:c1`：外层以太网帧，源/目标 MAC 为两台宿主机 `enp0s3` 网卡的 MAC
- `10.42.0.0.10384 > 10.42.1.7.443`：内层 TCP 包，这是 Pod 之间真实的通信——源 IP `10.42.0.0`，目标 IP `10.42.1.7`

> 注：抓包里的 `OTV` 是 tcpdump 的误识别——VXLAN 与 Cisco OTV 都默认用 UDP 8472 端口，老版本 tcpdump 会把 8472 上的 VXLAN 报文显示成 OTV，实际封装是 VXLAN。

### 2.3 Underlay: Host-GW 模式

{{< keypoint >}}
通路：宿主机路由表 + `cni0` 网桥——`flanneld` 将 Pod 子网路由写入宿主机路由表，跨节点无任何封装，性能等于裸机网络。
{{< /keypoint >}}

#### 2.3.1 组件概念

与前两种 Overlay 模式不同，Host-GW 采用**主机路由方案**——去掉的是隧道封装（没有 `flannel.1`、没有 `flannel0`），改走宿主机普通三层路由。既然跨节点不通是因为节点间缺少 Pod 子网的路由信息，而 Flannel 知道这些信息，就直接把它刷到宿主机路由表上。
...
然而，Flannel 只能修改**宿主机**的路由表，一旦节点间隔了其他三层设备（如路由器），该设备没有 Pod 子网的路由，会把包丢弃或错误转发。因此 Host-GW 要求所有节点在同一个 L2 广播域内——下一跳（目标节点）必须 on-link 直连可达。

#### 2.3.2 主要流程

{{< diagram >}}
工作原理：
1. 每个节点的 Pod 子网在 Etcd 注册，flanneld 感知全局子网
2. flanneld 在每台宿主机路由表写入到其他节点子网的静态路由，下一跳 = 目标节点主机 IP
3. 同节点通信：Pod → veth → cni0 网桥 → CAM 转发 → 目标 Pod（不进路由表）
4. 跨节点发送：源 Pod → veth → cni0 → 匹配远端 via 路由 → 经 eth0 发往目标节点
5. 目的节点接收：剥 L2 → 匹配本机直连路由 /24 dev cni0 → 进网桥 → CAM 转发 → 目标 Pod
{{< /diagram >}}
...
```text
发送侧（节点1）：
  10.244.1.5 → veth → cni0 → 匹配 10.244.2.0/24 via 192.168.1.2
  → 下一跳 = 192.168.1.2（on-link）→ ARP 拿节点2 eth0 的 MAC
  → 封 L2 帧（dst MAC = 节点2 eth0）→ 经 eth0 发出，IP 不变

接收侧（节点2）：
  eth0 收到 → 剥 L2 → 目的 IP = 10.244.2.8
  → 匹配本机直连路由 10.244.2.0/24 dev cni0（无 via，on-link）
  → ARP 拿 10.244.2.8（目的 Pod）的 MAC
  → 封 L2 帧（dst MAC = Pod MAC）→ 交给 cni0 网桥
  → cni0 查 CAM 表（MAC → 端口）→ 转发到对应 veth → 容器
```
