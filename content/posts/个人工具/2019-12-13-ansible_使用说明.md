...
sticky: 0
comments: true
description: 记录Ansible使用，包括环境准备、注意事项与操作记录
---

## 原理
Ansible 的工作原理是通过一台安装并配置了 Ansible 组件的主机（称为 `Ansible 控制节点`）来配置客户端机器（称为`受管节点`）。
...
> PPA（Personal Package Archive）: 个人软件包存档， 是 Ubuntu 系统中的概念。`PPA`提供了一种方便的方式来安装、更新和管理非官方的软件包，从而扩展了 Ubuntu 的软件生态系统。但用户需要自行评估软件包的可信度和稳定性，以确保安全和稳定性。
```bash
