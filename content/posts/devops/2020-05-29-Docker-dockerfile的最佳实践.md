...
| 操作                                              | 镜像大小 | 安全 | 构建性能 | 可读性 |
|-------------------------------------------------|------|----|------|-----|
| 尽可能使用官方Docker镜像;使用明确的镜像标签                       | 否    | 是  | 否    | 是   |
| `RUN、COPY、ADD`命令会创建layer，其他指令不会增加layer          | 是    | 否  | 否    | 否   |
| 始终在同一运行语句中结合运行 apt-get update 和 apt-get install | 否    | 否  | 是    | 否   |
| 使用`.dockerignore`排除构建不需要的文件。                    | 否    | 是  | 是    | 否   |
| 将长或复杂的RUN语句拆分到多行上，用反斜杠分隔。                       | 否    | 否  | 否    | 是   |
| 以非root用户运行；uid>10000；使用静态 UID 和 GID             | 否    | 是  | 否    | 否   |
| 使用静态 UID 和 GID                                  | 否    | 是  | 否    | 否   |
| 只在`CMD`中存储参数                                    | 否    | 否  | 否    | 是   |
| 使用COPY代替ADD                                     | 否    | 是  | 否    | 否   |
| 采取多阶段构建, 以减少最终运行镜像大小，环境clean。                   | 是    | 是  | 否    | 是   |

## 2. 最佳实践

- [尽可能使用官方Docker镜像](#尽可能使用官方docker镜像)
- [Alpine并不一定是最佳选择](#alpine并不一定是最佳选择)
- [使用明确的镜像标签，而非latest](#使用明确的镜像标签而非latest)
- [始终在同一运行语句中结合运行 apt-get update 和 apt-get install](#始终在同一run语句中运行apt-get-update和apt-get-install)
- [限制镜像layers数量](#限制镜像layers数量)
- [以非root用户运行](#以非root用户运行)
- [请勿使用低于`10000`的`UID`](#uid10000)
- [使用静态 UID 和 GID](#使用静态uid和gid)
- [只在`CMD`中存储参数](#只在cmd中存储参数)
- [使用COPY代替ADD](#使用copy代替add)
...
### 3.2 设置时区
> 时区原理以及具体说明，参考[理解容器时区问题的根源及常见解决方法]({{< ref "../kubernetes/2024-03-13-理解容器时区问题的根源及常见解决方法" >}})
...
#### 3.2.2 Ubuntu发行版
> ubuntu\debian\centos默认安装tzdata，因此只需设置环境变量，如果缺少tzdata包，请参考[理解容器时区问题的根源及常见解决方法]({{< ref "../kubernetes/2024-03-13-理解容器时区问题的根源及常见解决方法" >}})设置
